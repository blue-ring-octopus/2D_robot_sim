...
class EKF_SLAM:
    ...
    def predict(self, dt, u):
        ...
        if u[1]>=np.inf:#0.01:
            ...
            self.mu[0:3]+=np.asarray([dt*u[0]*np.cos(self.mu[2]+dt/2*u[1]),
                                 dt*u[0]*np.sin(self.mu[2]+dt/2*u[1]),
                                 dt*u[1]])
            self.mu[2]=angle_wrapping(self.mu[2])
            
            fx=np.eye(self.mu.shape[0])+F.T@np.asarray([[0,0,-dt*u[0]*np.sin(self.mu[2]+dt/2*u[1])], 
                                                        [0,0,dt*u[0]*np.cos(self.mu[2]+dt/2*u[1])],
                                                        [0,0,0]])@F
                           
            fu=np.asarray([[dt*np.cos(self.mu[2]+1/2*u[1]), -1/2*dt**2*u[0]*np.sin(self.mu[2]+dt/2*u[1])],
                           [dt*np.sin(self.mu[2]+1/2*u[1]), 1/2*dt**2*u[0]*np.cos(self.mu[2]+dt/2*u[1])],
                           [0, dt]])
            ...
            for z in feature:
                if z[3]:
                    if not str(z[2]) in self.feature.keys():
                        ...
                        self.mu=np.hstack((deepcopy(self.mu), loc))
                        sigma_new=np.diag(np.ones(self.sigma.shape[0]+2)*9999)
                        sigma_new[0:self.sigma.shape[0], 0:self.sigma.shape[0]]=deepcopy(self.sigma)
    ...
    def update(self,dt,z, u):
        self.predict(dt, u)
        self.estimate(dt,z)
        return deepcopy(self.mu), deepcopy(self.sigma), []
