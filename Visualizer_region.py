...
class Visualizer:
    def __init__(self,world, plot_resolution, region):
        self.world=world
    ...
    def visualize(self):
        ...
        region=self.region.get_region(world.robots[0].x[0], world.robots[0].x[1], world.robots[0].x[2])
        print(region)
        k=int((-np.pi/2-world.robots[0].x[2])/(2*np.pi/50))
        regions=cv2.resize((self.region.colorgrid[:,:,k,:]*255).astype(np.uint8), (600,600), interpolation=cv2.INTER_NEAREST )
        mask = ~(regions==np.array([0,0,0])).all(-1)
        image[mask]=cv2.addWeighted(image[mask], 0.5,regions[mask], 0.5, 0)
        ...
        image=cv2.copyMakeBorder(image, 0, 0, 0, 200, cv2.BORDER_CONSTANT)
        image = cv2.putText(image, 'Region:'+str(region), org = (610, 185), 
                   fontScale = 0.8, color=(255,255,255), thickness = 2, fontFace=cv2.LINE_AA)
